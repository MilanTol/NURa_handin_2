...
I found it hard to find an analytic inverse of the cumulative distribution, nor did I find a function 
that fully encloses p for all x and for all other parameters that p depends on, that did not enclose a 
large chunk of probability space not enclosed by p. Besides, evaluating p is quite fast. So for our current 
means and purposes I use rejection sampling, even though the
distribution is quite peaked, which makes rejection sampling a lot more inefficient.

The reason rejection sampling becomes so inefficient is because rejection sampling 
samples points randomly in an area equal to $p_{max} \times \mathrm{domain range}$. But 
a probability distribution is normalized so it only contains an area of $1$.
So for very peaked distributions (where $p_{max} >> 1/\mathrm{domain range}$), we have that
$(p_{max} \times \mathrm{domain range} - 1) * N \approx N$ samples get rejected when drawing N samples.

The total rejected samples for this run was: \input{Calculations/rejected_samples.txt}.
So in an ideal world where we would have access to the inverse cumulative distribution
or a function that tightly encloses p (for all parameters), sampling this distribution would be over 10 times faster.

We sample 10000 radii and plot the points in 20 logspace-bins, weighted by the inverse binsize. 
We see that the analytic prediction matches the sampled distribution quite well for high probability regions.
However, for regions where the probability of ending up in is smaller than $1/10000$, the sampled distribution
does not provide any information.

The random number generator used for sampling uses a combination of 3 random processes:
2 XOR operations and 1 MWC operation. Each of these processes carries their own state.
I have tested the RNG using diehard, I installed the latest version through sudo apt on linux.
I tested the rng for $10^7$ integers for seed of 1.
The RNG passed most tests but failed and was weak for some others. 
(I have tried multiple xor and MWC combinations but I couldnt find one that passes all, this one seemed best).
See the output of diehard below.

\begin{verbatim}
# The file file_input_raw was rewound 1 times
   diehard_birthdays|   0|       100|     100|0.81924178|  PASSED  
# The file file_input_raw was rewound 11 times
      diehard_operm5|   0|   1000000|     100|0.01177582|  PASSED  
# The file file_input_raw was rewound 24 times
  diehard_rank_32x32|   0|     40000|     100|0.00005522|   WEAK   
# The file file_input_raw was rewound 30 times
    diehard_rank_6x8|   0|    100000|     100|0.19898295|  PASSED  
# The file file_input_raw was rewound 32 times
   diehard_bitstream|   0|   2097152|     100|0.96039054|  PASSED  
# The file file_input_raw was rewound 53 times
        diehard_opso|   0|   2097152|     100|0.00058022|   WEAK   
# The file file_input_raw was rewound 67 times
        diehard_oqso|   0|   2097152|     100|0.28602377|  PASSED  
# The file file_input_raw was rewound 74 times
         diehard_dna|   0|   2097152|     100|0.48982144|  PASSED  
# The file file_input_raw was rewound 74 times
diehard_count_1s_str|   0|    256000|     100|0.75092656|  PASSED  
...
# The file file_input_raw was rewound 88 times
    diehard_2dsphere|   2|      8000|     100|0.39575233|  PASSED  
# The file file_input_raw was rewound 88 times
    diehard_3dsphere|   3|      4000|     100|0.95908078|  PASSED  
# The file file_input_raw was rewound 111 times
     diehard_squeeze|   0|    100000|     100|0.00000003|  FAILED  
# The file file_input_raw was rewound 111 times
        diehard_sums|   0|       100|     100|0.15557686|  PASSED  
# The file file_input_raw was rewound 112 times
        diehard_runs|   0|    100000|     100|0.41121387|  PASSED  
        diehard_runs|   0|    100000|     100|0.56491658|  PASSED  
# The file file_input_raw was rewound 125 times
       diehard_craps|   0|    200000|     100|0.00000000|  FAILED  
       diehard_craps|   0|    200000|     100|0.22382397|  PASSED 
\end{verbatim}
...
For this we use the quicksort algorithm, since we do not need a stable algorithm, but do care about speed and might 
potentially want to scale up the problem in the future. 
...
To numerically compute the derivative we employ Ridder's algorithm. 
This algorithm uses a sequence of finite difference differentiation attempts and extrapolates 
these estimates to $h \rightarrow 0$. Where h is the stepsize used for finite difference. 
This algorithm is extremely powerful if the function is sufficiently smooth and is close to linear
over the ranges of $h$ considered for the initial estimates.
The analytic and numeric values of the derivative of the number density of satellites can be seen 
in the table below.
\begin{table}[H]
\centering
...
The analytical and numerical derivative agree to very high precision. 
This is because the number density of satellites is a product and composition of smooth functions.
So when taking sufficiently small h, ridders algorithm returns an extremely precise answer.
...
I hypothesize that false position will be the fastest since this is a safe algorithm that
assumes that the function is approximately linear. 
At a first glance the function above seems to be quite linear, since the logarithm isnt a 
sharply increasing function. I dont think Newton-Raphson will do well (explanation is below).
...
It turns out that Newton-Raphson does not converge for most starting values in our case.
In order for this method to be effective, the starting point needs to be \textit{sufficiently} close to the true root.
That is why I instead use an improved Newton-Raphson method: Use Newton-Raphson as long as the new proposed best estimate lies
within a bracket. If it does not, resort to false position (and potentially even bisection). It turns out that this 
algorithm is the fastest and most accurate.
...
\hline
Name & Temperature [K] & Absolute error & Relative error & iterations & time to complete \\
\hline
...
We see that newton-raphson manages to find the exact root within 6 iterations. Which is quite a bit better
than False position and a lot better than bisection. However, since the coded algorithm requires a lot more checks 
and also calls the derivative function, it requires a lot more time to run per iteration. Only making it twice as fast 
as bisection and false position. All algorithms converge to the correct answer. 

\subsection{Code for finding roots}
\lstinputlisting[language=Python]{\CodeDir/root_finders.txt}



\subsection{(2b) More realistic model}

In this problem we are asked to find the root of a more complicated funtion for various values of the hydrogen density.
We compare the same algorithms as before. Since bisection is consistent, this is our reference as to whether 
the algorithms produce correct answers. In this case we see that only for low density does the false position and improved Newton-Raphson produce the correct answer
but it has a lot more trouble finding it compared to bisection and hence is substantially slower. 
However the error does not decrease properly so it thinks it does not achieve the correct tolerance.
There is probably a better implementation of the algorithm such that it does estimate the error correctly.
A possible solution is to make an estimator of the error based on how much c varies at each iteration, However
the drawback of such an implementation is that the algorithm may significantly underestimate the error
if it gets stuck at a point that is not an actual root.
For the other densities the methods simply do not converge to the correct result.
A possible solution to relieve this error is to find a more compact bracket using bisection and only then use the 
false position and improved newton-raphson method.

We see that for larger densities the equilibrium temperature drops. 
For a small hydrogen density the temperature is enormous. So in all realistic cases,
these clouds are not in equilibrium and other mechanisms will adjust the density accordingly.

\begin{table}[H]
\centering
\caption{Equilibrium temperature for $n_H = 10^{-4}$ [cm\(^{-3}\)]}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
Name & Temperature [K] & Absolute error & Relative error & iterations & time to complete \\
\hline
\input{\CalcDir/equilibrium_low_density_bisection.txt} \\
\input{\CalcDir/equilibrium_low_density_false_position.txt} \\
\input{\CalcDir/equilibrium_low_density_newton_raphson.txt} \\
\hline
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Equilibrium temperature for $n_H = 1$ [cm\(^{-3}\)]}
\begin{tabular}{|c|c|c|c|c|c|}
\hline
Name & Temperature [K] & Absolute error & Relative error & iterations & time to complete \\
\hline
\input{\CalcDir/equilibrium_mid_density_bisection.txt} \\
\input{\CalcDir/equilibrium_mid_density_false_position.txt} \\
\input{\CalcDir/equilibrium_mid_density_newton_raphson.txt} \\
\hline
\end{tabular}
\end{table}
...
Name & Temperature [K] & Absolute error & Relative error & iterations & time to complete \\
\hline
\input{\CalcDir/equilibrium_high_density_bisection.txt} \\
\input{\CalcDir/equilibrium_high_density_false_position.txt} \\
\input{\CalcDir/equilibrium_high_density_newton_raphson.txt} \\
\hline
\end{tabular}
\end{table}
