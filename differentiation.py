...
def ridders_derivative(
    ...
    for i in range(1, max_iters):
        ...
        if temp_err < err:  # if error shrinks we update
            ...
            if err < np.abs(
                eps * best
            ):  # check whether relative error is smaller than target
                return best

            D_last[: i + 1] = D[
                : i + 1
            ]  # store current added estimates into D_last for next iteration

        else:  # if error grows terminate early, cant cancel roundoff errors
            print(
                "WARNING: target error-tolerance could not be achieved, "
